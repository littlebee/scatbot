...
import { getStateFromCentralHub } from "./hub-state";
        ...
        <div className={classnames(st.dialogContent, closedClass)}>
          {isOpen && (
            <div className={classnames(st.dialogViewport)}>
              <div className={classnames("buttons", st.videoSelector)}>
                <Button
                  isSelected={whichState === LOCAL_STATE}
                  onClick={() => handleStateChange(LOCAL_STATE)}
                >
                  local hub state
                </Button>
                <Button
                  isSelected={whichState === REMOTE_STATE}
                  onClick={() => handleStateChange(REMOTE_STATE)}
                >
                  remote hub state
                </Button>
              </div>
